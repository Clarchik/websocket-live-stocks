...
@Injectable()
export abstract class BaseStockService extends AbstractStockService {
  protected readonly _stocks = signal<StockState[]>(
    ...
    this._stocks.update((stocks) =>
      stocks.map((s) => {
        ...
        const trend: StockState['trend'] =
          update.currentPrice > s.quote.currentPrice
            ? 'up'
            : update.currentPrice < s.quote.currentPrice
              ? 'down'
              : 'neutral';
  ...
  toggleStock(symbol: string): void {
    this._stocks.update((stocks) =>
      stocks.map((s) =>
        s.quote.symbol === symbol ? { ...s, active: !s.active, trend: 'neutral' as const } : s,
      ),
    );
  }
}
